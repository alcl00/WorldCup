...
echo "$($PSQL "SELECT name FROM teams WHERE team_id in (SELECT winner_id FROM games WHERE year=2014 AND round='Eighth-Final') OR team_id in (SELECT opponent_id FROM games WHERE year=2014 AND round='Eighth-Final') ORDER BY name ASC")"
